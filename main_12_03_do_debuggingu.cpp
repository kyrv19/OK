...
int pokoloruj(lista **tab_incydencji, int *kolejnosc, int *wynik, int N)
{
    int *kopia = new int [N];
...
bool listaTabu_compare(listaTabu *tabu, instancja *insta, int N)    //return false jezeli rozne
{
    ...
    else
    {
        do
        {
            for(int i = 0; i < N; i++)
            {
                if(temp_insta->kolejnosc[i] != insta->kolejnosc[i])
                {
                    return false;
                }
            }
...
int wybierzBestPermutacje(lista **tab_incydencji,int *kolejnosc, int *pokolorowanie, int N, int *kolejnosc_zwracana)
{
...
int tabu(lista **tab_incydencji, int *kolejnosc, int *pokolorowanie, int N)
{
    ...
    while(dzialaj < 5)
    {
        ...
        int bestCandidateLiczbaKolorow = wybierzBestPermutacje(tab_incydencji, bestCandidate->kolejnosc, bestCandidate->pokolorowanie, N, bestCandidateKolejnosc);
...
int main()
{
    ...
    int *kopia = new int [N];
    copy(pokolorowanie, pokolorowanie + N, kopia);
    cout<<"best best pierwszy: "<<pokoloruj(tab_incydencji, rosnace, pokolorowanie, N)<<endl;
    cout<<szukajMaxKolor(pokolorowanie, N)<<endl;
    for(int j = 0; j < N; j++)
    {
        cout<<pokolorowanie[j]<<", ";
    }
    cout<<endl;
    bool error = false;
    for(int j = 0; j < N; j++)
    {
        if(pokolorowanie[j] != kopia[j]) {error = true;}
    }
    if(error) {cout<<"erroe!!\n\n\n\n\n"<<endl<<endl;}
    cout<<endl;
    copy(pokolorowanie, pokolorowanie + N, kopia);

    cout<<"best best drugi: "<<pokoloruj(tab_incydencji, rosnace, pokolorowanie, N)<<endl;
    cout<<szukajMaxKolor(pokolorowanie, N)<<endl;
    for(int j = 0; j < N; j++)
    {
        cout<<pokolorowanie[j]<<", ";
    ...
    cout<<"best best trzeci: "<<pokoloruj(tab_incydencji, rosnace, pokolorowanie, N)<<endl;
    cout<<szukajMaxKolor(pokolorowanie, N)<<endl;
